...
class LearningWindow(QMainWindow):
    def __init__(self, token):
        ...
        flo2.addRow("Path to aggregation script: ",self.line_test)
        vbox2.addLayout(flo2)

        self.button_upload_learning = QPushButton('Select aggregation script',self)
        ...
        flo3.addRow("Path to aggregation script: ",self.line_script)
        vbox2.addLayout(flo3)

        self.button_upload_weights = QPushButton('Select downloaded weights folder',self)
        ...
        self.label_weights = QLabel("Path to downloaded weights: ")
        ...
        groupbox = QGroupBox('Aggregation')
        ...
        self.button_results = QPushButton('Select where to store aggregated weights',self)
        ...
        self.label_results = QLabel("Path to aggregated weights: ")

        flo5.addRow(self.label_results,self.line_results)
        vbox.addLayout(flo5)

        self.button_learn = QPushButton('Run aggregation',self)
        self.button_learn.setEnabled(False)
        self.button_learn.clicked.connect(self.run_aggregating)
    ...
    def get_testing_folder(self):
        ...
        if folderpath:
            ...
            is_enabled = self.isTestUploaded and self.isScriptUploaded and self.isWeightUploaded
            self.button_learn.setEnabled(is_enabled)
            self.line_test.setText(folderpath)
            # self.line_test.adjustSize()
            print(self.line_test.text())
    ...
    def get_weights_path(self):
        ...
        if folderpath:
            ...
            is_enabled = self.isTestUploaded and self.isScriptUploaded and self.isWeightUploaded
            self.button_learn.setEnabled(is_enabled)
            self.line_weights.setText(folderpath)
            # self.line_test.adjustSize()
            print(self.line_weights.text())

    def run_aggregating(self):
        ...
        print(weights)
        print(absolute_path)
        aggregator.aggregate(self.token,testpath, absolute_path, weights)
